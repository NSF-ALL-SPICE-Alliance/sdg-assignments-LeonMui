...
# adding plotly package to create interactive graphs
...
```{r, warning=FALSE, message=FALSE}
# changing the code chunk from SDG scores 9 and 12 to...

linear_regression_of_sdg_3_and_8 <- ggplot(sdr_data, aes(x = goal_3_score, 
...
ggplotly(linear_regression_of_sdg_3_and_8)

```

Hypothesis: 
There is a general positive correlation between the SDG 3 (Good Health and Well-Being) score and the SDG 8 (Decent Work and Economic Growth) score. With good health and well-being, it usually results in a stronger and healthier workforce, and vice versa (such as Singapore, Norway, and Australia). However, there are outliers in the countries such as South Sudan, Fiji, other countries in the Middle East (i.e. Lebanon, Iran, etc.), and regions in Oceania and Africa. The results may be linked imbalances in the countries' health and economy due to instability in the region or lack of access to healthcare resources. 
