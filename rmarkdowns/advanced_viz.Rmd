...
```{r, warning = FALSE}
# edited code chunk to name the histogram and create a theme with a color palette and appropriate labels

goals_3_and_4_histogram <- ggplot(data = sdr_data, aes(x= goal_3_score, y = goal_4_score, color = regions_used_for_the_sdr)) +
  geom_point() +
  theme_minimal () +
  scale_fill_viridis_d() +
...
ggplotly(goals_3_and_4_histogram)
```
...
```{r}
# changing the map from SDG 7 to SDG 1
...
# changing the color palette from "YlOrRd" to "YlGnBu" and adjusting for SDG 1 scores

leaflet(sdr_data_world_joined) %>% 
  addTiles()  %>% 
  setView( lat=10, lng=0 , zoom=2) %>%
  addPolygons(stroke = FALSE, fillOpacity = 0.5, smoothFactor = 0.5, color = ~colorQuantile("YlGnBu", goal_1_score)(goal_1_score), label = mytext)
```
