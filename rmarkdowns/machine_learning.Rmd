...
```{r}
# Edited k2 function to have 3 clusters
...
```{r}
# Edited to make a model of SDG 3 Obesity predictions
sdg_2_obesity <- randomForest(normalized_score_sdg2_obesity ~ .,
...
```{r}
# edited for accuracy of new model
importance_df <- as.data.frame(sdg_2_obesity$importance)
```
...
```{r}
# edited for new model
pdp::partial(rf_matmort, pred.var = "normalized_score_sdg7_cleanfuel", plot = TRUE)
```
...
```{r}
# edited for new model
pd <- pdp::partial(rf_matmort, pred.var = c("normalized_score_sdg7_cleanfuel", "normalized_score_sdg3_tb"))
# Default PDP
plotPartial(pd)
```
