...
```{r}
ggplot(data = sdr_data, aes(x= goal_6_score, y = goal_7_score, color = goal_8_score)) +
  geom_point() # created a scatterplot using different SDG scores from the example shown
```

Interpretation: 
In the upper right quadrant we can see a positive correlation with an almost linear trend line, indicating that some countries who are doing well in the Goal 8 score tend to also do well in the Goal 6 and 7 scores. However, this isn't the case for some countries.
...
ggplot(mena_sdr_data, aes(x= goal_3_score, y = country)) +
  geom_bar(stat = "identity") # creating a bar chart using only countries from the MENA region
``` 
...
ggplot(sdr_data, aes(x = goal_3_score, fill=regions_used_for_the_sdr)) + 
  geom_histogram(bins=33, color="black") # created a histogram for the SDG 3 score
```

Interpretation: 
Countries that fall in the higher end of the Goal 3 spectrum tend to belong to the OECD region, while countries in Sub-Saharan Africa lie towards the lower end.
